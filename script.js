...
fetch('airports.json').then(res => res.json()).then(data => { const airportDataArray = Object.values(data); airportLookup = new Map(airportDataArray.map(airport => [airport.icao, airport])); initializeGlobe(); }).catch(error => { console.error(error); globeContainer.innerHTML = `<div style="text-align: center; padding: 50px; color: white; background: #0f202b;"><h2>CRITICAL ERROR</h2><p>Could not load essential airport data (airports.json).</p></div>`; });